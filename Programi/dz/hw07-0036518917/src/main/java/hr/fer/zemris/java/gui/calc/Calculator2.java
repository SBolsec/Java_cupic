...
public class Calculator2 extends JFrame {
	...
	private void initUnaryButtons() {
		...
		for (int i = 0; i < 7; i++) {
			...
			btn.addActionListener(l -> {
				double res = 0;
				try {
					res = btn.getOperator().applyAsDouble(model.getValue());
				} catch (Exception e) {}
				if (model.isActiveOperandSet() && model.getPendingBinaryOperation() == null) {
					res = btn.getOperator().applyAsDouble(model.getActiveOperand());
	...
	private void initBinaryButtons() {
		...
		for (int i = 0; i < 4; i++) {
			...
			final int op = i;
			btn.addActionListener(l -> {
				binaryButtonAction(operators[op]);
			});
		...
		btn.setText("x^n");
		btn.addActionListener(l -> {
			binaryButtonAction(btn.getOperator());
		});
		cb.addActionListener(btn);
	...
	private void initOtherButtons() {
		...
		push.addActionListener(l -> {
			...
			} catch (Exception e) {JOptionPane.showMessageDialog(null, "Push failed!");}
		});
